...
export function useIngestWithDuplicateHandling({
  ...
  const openSkippedDialog = useCallback(
    (
      ...
      const { files, syncPublishedVisible } = selectFilesForConflicts(
        originalPayload.files,
        Array.isArray(originalPayload.sync_published_visible)
          ? originalPayload.sync_published_visible
          : null,
        conflicts,
      );
      ...
      setPendingPayload({
        ...
        sync_published_visible:
          syncPublishedVisible ?? originalPayload.sync_published_visible,
