...
import { useAdminModuleDetailQuery } from '@/features/modules/hooks/useAdminModuleDetailQuery';
...
export function useAdminModuleReviewEditor(moduleId: string) {
      ...
      const saveTask = (async () => {
        ...
        try {
          ...
          const nextWorking = detailsOverride
            ...
            : working;
          try {
            return await persistAdminModuleDraft({
              working: nextWorking,
              editModule,
              navigate,
              pathname,
            ...
            const conflict = parseModuleVersionConflictError(err);
            if (conflict) {
              dispatch(setVersionConflict(conflict));
            }
            throw err;
          }
        } finally {
  ...
  const formatError = useCallback((error: unknown) => {
    if (parseModuleVersionConflictError(error)) {
      return '';
    }
