...
export const ModuleAssignmentDialog = ({
  ...
  const [createAssignment, { isLoading: isAssigning }] =
    useCreateAssignmentMutation();
  const [revokeAssignment, { isLoading: isRevoking }] =
    useRevokeAssignmentMutation();
  ...
  const handleAssign = async () => {
    ...
    try {
      if (activeTab === 'user') {
        ...
        await Promise.all(
          assignmentIdsToRevoke.map((assignmentId) =>
            revokeAssignment(assignmentId).unwrap(),
          ),
        );
        ...
        if (userIdsToAssign.length > 0) {
          ...
          if (result.assigned_count === 0) {
            setErrorMsg(
              'This module is already assigned to the selected user(s). Choose different users or check existing assignments.',
            );
            return;
          }
        }

        // Success page should show only newly assigned users by default.
        ...
        navigateToSuccess(assignmentType, assignedUsers, removedUsers);
        return;
      }
      ...
      if (activeTab === 'geographical') {
        ...
        // Success page should show only newly assigned upazilas by default.
        const assignedUsers =
          buildGeographicalAssignedEntries(upazilasToAssign);

        navigateToSuccess('geographical', assignedUsers, removedUsers);
      }
    } catch (err: unknown) {
      console.error(err);
      const detail =
  ...
  return (
    <Modal open={open} labelledBy="assignment-dialog-title" onClose={onClose}>
      <Loader
        open={isSubmitting}
        label={isAssigning ? 'Assigning module…' : 'Revoking assignment…'}
      />
      <Card
        variant="elevated"
