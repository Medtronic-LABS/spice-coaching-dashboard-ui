...
export function mapModulePipelineToCourseDraft(
  ...
  const lessons: CourseDraftData['lessons'] = lessonsSource.map(
    (lesson, index) => ({
      id: `lesson_${index + 1}`,
      title: lesson.title,
      order: index + 1,
  ...
  const questions: CourseDraftData['quiz']['questions'] = questionsRaw.map(
    (q) => {
  ...
  const quiz: CourseDraftData['quiz'] = {
