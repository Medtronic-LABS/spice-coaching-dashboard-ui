...
} from '@/components/ui/rich-text/tiptap/cardMediaAttrs';
import { parsePositiveDimension } from '@/features/modules/utils/cardImageDimensions';
...
function imageDisplayAttrsToTiptap(attrs: RichImageBlock['attrs']): {
  width: number | null;
  height: number | null;
} {
  return {
...
function imageToTiptap(node: RichImageBlock): JSONContent {
  return {
    type: 'image',
    attrs: {
      ...storedFileAttrsToTiptap(node.attrs),
      ...imageDisplayAttrsToTiptap(node.attrs),
    },
  };
}
...
function imageFromTiptap(node: JSONContent): RichImageBlock | null {
  ...
  const fileAttrs = storedFileAttrsFromTiptap(readCardMediaTiptapAttrs(node));
  if (!hasStoredFileReference(fileAttrs)) return null;
  const rawAttrs = node.attrs ?? {};
  const width = parsePositiveDimension(rawAttrs.width);
  const height = parsePositiveDimension(rawAttrs.height);
  return {
    type: 'image',
    attrs: {
      ...fileAttrs,
