...
} from '@/components/ui/rich-text/constants/cardMediaAccept';
import { CardImageExtension } from '@/components/ui/rich-text/tiptap/CardImageExtension';
import { storedFileAttrsToTiptap } from '@/components/ui/rich-text/tiptap/cardMediaAttrs';
...
export interface RichTextEditorProps {
  ...
  onEditorFocus?: () => void;
  /**
   * Feature-owned media upload. Keep API calls out of this UI primitive.
   * Return stored file attrs for the uploaded object.
   */
  onUploadMediaFile?: (file: File) => Promise<RichStoredFileAttrs>;
...
export const RichTextEditor = ({
  ...
  const onUploadMediaFileRef = useRef(onUploadMediaFile);
  ...
  onUploadMediaFileRef.current = onUploadMediaFile;
  ...
  const insertUploadedFile = async (file: File, pos?: number) => {
    ...
    const uploadMediaFile = onUploadMediaFileRef.current;
    if (!uploadMediaFile) {
      setMediaError('Media upload is unavailable in this editor.');
      return;
    }
    ...
    setIsUploading(true);
    try {
      const fileAttrs = storedFileAttrsToTiptap(await uploadMediaFile(file));
